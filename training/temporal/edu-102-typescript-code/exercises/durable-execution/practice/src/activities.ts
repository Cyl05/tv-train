...
export async function translateTerm(input: TranslationActivityInput): Promise<TranslationActivityOutput> {
  ...
  context.log.info('Translating ', { LanguageCode: input.languageCode, Term: input.term });
  ...
  try {
    ...
    if (error.response) {
      ...
      context.log.error('Error: ', { status: error.response.status, data: error.response.data });
      ...
      context.log.error('Translation Error:', { request: error.request });
